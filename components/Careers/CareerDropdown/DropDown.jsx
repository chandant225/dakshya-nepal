          ...
          <AccordionItemPanel>
            <div className="tab-content overflow-hidden bg-gray-50 leading-normal border border-t-[2px] border-gray-300">
              <div className=" flex font-primary flex-col p-[24px]">
                ...
                <div className="mb-[12px] text-sm text-gray-400">
                  {" "}
                  {htmr(info.specifications, { transform })}{" "}
                </div>
                <p className="mb-[12px] text-base font-medium ">
                  Job Responsibilities
                </p>
                <div
                  className="mb-[12px] text-sm text-gray-400"
                  style={{ listStylePosition: "inside !important" }}
                >
                  {" "}
                  {htmr(info.responsibilites, { transform })}{" "}
                </div>
                <button
                  label="Apply"
                  onClick={() => setApply(true)}
                  className="border-solid border-[3px] border-black bg-white hover:bg-black text-black hover:text-white font-bold py-1.5 md:py-2 px-auto rounded-md mb-[6px] w-[80px] transition-all ease-linear duration-300 "
                >
                  Apply
                </button>
                {apply && (
                  <ApplyJob setApply={setApply} title={info.position} />
                )}
              </div>
            </div>
          </AccordionItemPanel>
